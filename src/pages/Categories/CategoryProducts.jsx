...
export default function CategoryProducts() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () =>
            api.get(`/products?category=${categoryId}&page=${page}&limit=12`),
        select: (res) => res.data,
        enabled: !!categoryId,
    });
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50">
            ...
            <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 via-slate-800 to-violet-900 text-white">
                <div className="absolute -top-20 -right-20 w-80 h-80 bg-violet-400 opacity-10 rounded-full blur-3xl pointer-events-none" />

                <div className="relative container py-12 flex flex-col gap-4">
                    {/* breadcrumb */}
                    <nav className="flex items-center gap-2 text-sm text-slate-400">
                        <Link to="/categories" className="hover:text-violet-400 transition-colors">
                            Categories
                        </Link>
                        <span>/</span>
                    ...
                    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                        <div>
                            <span className="inline-flex items-center gap-2 bg-violet-500/20 border border-violet-400/30 text-violet-300 text-xs font-semibold tracking-widest uppercase px-3 py-1 rounded-full mb-3">
                        ...
                        {products.length > 0 && (
                            <div className="flex flex-col items-start md:items-end shrink-0">
                                <span className="text-3xl font-extrabold text-violet-400">{products.length}</span>
            ...
            <div className="container py-10">
                ...
                {products.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                        ...
                        <p className="text-slate-400 text-sm">This category doesn't have any listed products yet.</p>
                        <Link
                            to="/categories"
                            className="mt-2 inline-flex items-center gap-2 bg-violet-600 text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-violet-700 transition-colors"
                        >
                            ← Back to Categories
                        </Link>
                    </div>
