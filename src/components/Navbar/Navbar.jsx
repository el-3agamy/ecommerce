...
const menuItems = ['Home', 'Categories', 'Brands', 'Wishlist', 'Cart'];
...
const navLinkClassFull = ({ isActive }) =>
  `${navLinkClass({ isActive })} w-full`;

const itemPath = (item) => (item === 'Home' ? '/' : '/' + item.toLowerCase());
  ...
  return (
    <Navbar onMenuOpenChange={setIsMenuOpen} isMenuOpen={isMenuOpen} shouldHideOnScroll isBordered>
      <NavbarContent>
        ...
        />
        <NavbarBrand>
          <p className="font-bold text-inherit hover:text-green-500">
            <i className="fa-solid fa-cart-shopping text-green-500 text-xl align-middle" /> FreshCart
          </p>
        </NavbarBrand>
      </NavbarContent>
        ...
        <NavbarContent className="hidden sm:flex gap-4" justify="center">
          {menuItems.map((item, index) => (
            <NavbarItem key={index}>
              <NavLink className={navLinkClass} to={itemPath(item)}>
                {item}
              </NavLink>
            </NavbarItem>
          ))}
        ...
        <NavbarMenu>
          {menuItems.map((item, index) => (
            <NavbarMenuItem onClick={() => setIsMenuOpen(false)} key={`${item}-${index}`}>
              <NavLink className={navLinkClassFull} to={itemPath(item)}>
                {item}
              </NavLink>
            </NavbarMenuItem>
          ))}
